...
export function showGenerateModal(app) {
  if (!app.jamoGrid?.isAllCompleted()) {
    ...
    showToast(`검토하거나 내보내려면 필수 자모 ${remaining}개를 더 완성해야 합니다.`);
    return;
  }
  ...
  overlay.innerHTML = `
    <div class="modal generate-modal">
      ...
      <div class="modal-body">
        ...
        <p class="gen-hint" id="genHint">폰트 이름을 바꾼 뒤에는 다시 생성해야 그 이름으로 다운로드됩니다.</p>
      </div>
    </div>
  `;
...
function syncDownloadGate(app) {
  ...
  if (gateMeta) {
    const status = unlockedForCurrent
      ...
      : `<span class="download-gate-badge is-locked">잠금</span><span>현재 이름 기준으로는 아직 결제하지 않았습니다. 테스트 가격 9,900 KRW</span>`;
